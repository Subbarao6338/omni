...
def get_file_metadata(file_path):
    ...
    # print(f"Fetching commits from: {commits_url}")  # Debugging line
    response = requests.get(commits_url, headers=headers)
    response.raise_for_status()

    commits = response.json()
    # print(f"Commits response: {commits}")  # Debugging line
    ...
    for item in items:
        ...
        if item['type'] == 'file':
            if item['name'].endswith('.py') and total_python_files_downloaded < MAX_PYTHON_FILES:
                download_file(item['download_url'], PYTHON_DIR, item['name'])
            elif item['name'].endswith(('.txt', '.md')) and total_text_files_downloaded < MAX_TEXT_FILES:
                download_file(item['download_url'], TEXT_DIR, item['name'])
        elif item['type'] == 'dir':
            subdirectory_url = f"{os.getenv('GITHUB_REPO')}{GITHUB_REPO}/contents/{item['path']}"
...
# Display the contents of the CSV file to the console
print("\nFile Metadata:")
with open(csv_file_path, 'r') as csvfile:
    reader = csv.DictReader(csvfile)
    # for row in reader:
    #     print(row)
    table = [row for row in reader]  # Create a list of rows for tabulate
    # print(tabulate(table, headers="keys", tablefmt="grid"))

    print("\nFile Metadata (Plain):")
    print(tabulate(table, headers="keys", tablefmt="plain"))

    print("\nFile Metadata (Simple):")
    ...
    print("\nFile Metadata (Fancy Grid):")
    print(tabulate(table, headers="keys", tablefmt="fancy_grid"))

    print("\nFile Metadata (HTML):")
    print(tabulate(table, headers="keys", tablefmt="html"))

    print("\nFile Metadata (orgtbl):")
    print(tabulate(table, headers="keys", tablefmt="orgtbl"))

    print("\nFile Metadata (mediawiki):")
    print(tabulate(table, headers="keys", tablefmt="mediawiki"))
    ...
    print("\nFile Metadata (jira):")
    print(tabulate(table, headers="keys", tablefmt="jira"))
