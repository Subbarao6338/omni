...
def process_files(github_files, downloaded_files):
    ...
    for file_data in github_files:
        if file_data["type"] == "blob":
            ...
            clean_file_name = clean_file_name.lstrip('0123456789_')  # Remove numeric prefixes and underscores

            # Handle duplicate file names
            if clean_file_name in file_counter:
                file_counter[clean_file_name] += 1
                clean_file_name = f"{clean_file_name}_{file_counter[clean_file_name]}"
            else:
                file_counter[clean_file_name] = 1
            ...
            local_file_path = os.path.join(local_dir, f"{BRANCH_NAME}_{dir_structure}_{clean_file_name}_{commit_timestamp}")

            unique_file_id = (REPO_NAME, clean_file_name, commit_timestamp)

            print(f"Processing file: {file_path_in_repo}")
            # print(f"Unique file ID: {unique_file_id}")
