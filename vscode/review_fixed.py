...
def run_pylint(file_path):
    """Run Pylint on a given file and return the output, errors, rating, and exit code."""
    try:
        ...
        return output, errors, rating, exit_code  # Return the output, captured errors, rating, and exit code
    except Exception as e:
        print(f"Error running pylint on {file_path}: {e}")  # Print error message if an exception occurs
...
def generate_report(file_reports, output_file):
    ...
    with open(output_file, "w") as md_file:
        ...
        total_files = len(file_reports)  # Total number of files reviewed
        files_with_issues = sum(1 for report in file_reports.values() if report["exit_code"] != 0)  # Count files with issues

        # Write summary statistics to the report
        ...
        for file_path, report in file_reports.items():
            ...
            if report["exit_code"] != 0:
                ...
                if report.get("errors"):
                    md_file.write("#### Errors:\n```\n" + report["errors"] + "\n```\n\n")


def main():
    """Main function to execute the Pylint review process."""
    # Prompt user for the directory path to review
    ...
    for file_path in python_files:
        ...
        if output is not None:
            ...
            file_reports[str(file_path)] = {
                ...
                "errors": errors,  # Store any errors (currently not captured in run_pylint)
    ...
    generate_report(file_reports, report_file)  # Generate the Markdown report
    print(f"Review completed. Report saved to '{report_file}'.")  # Notify the user that the report is saved
