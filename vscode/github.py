...
def main():
    # Fetch files from the base URL
    contents = fetch_files(BASE_URL)
    files_metadata = []

    # Count total files to process
    total_files = sum(1 for item in contents if item['type'] == 'file' and (
                item['name'].endswith('.py') or item['name'].endswith('.txt') or item['name'].endswith('.md')))
    ...
    for item in contents:
        if item['type'] == 'file':
            ...
            if local_path and item_info:
                ...
                print(f"\rProcessing files: {progress:.2f}% complete", end='')
