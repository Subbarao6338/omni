...
def get_file_metadata(file_path):
    ...
    headers = {'Authorization': f'token {GITHUB_TOKEN}'}
    # Construct the commits URL with the branch name
...
def download_file(file_url, save_dir, file_name, github_path):
    ...
    # Prefix the branch name to the file name
    base_name, extension = os.path.splitext(file_name)
    new_file_name = f"{BRANCH_NAME}_{base_name}{extension}"  # Prefix branch name

    # Check if the file already exists in the target directory
    local_file_path = os.path.join(save_dir, new_file_name)

    if os.path.exists(local_file_path):
        print(f"Skipping {local_file_path} as it already exists.")
        # Add metadata for the skipped file
    ...
    # Get the metadata for the downloaded file using the original github_path
    creation_date, commit_message = get_file_metadata(github_path)
    metadata.append({
        'File Location': local_file_path,
        'File URL': normalized_url,
    ...
    # Add the normalized URL to the set of downloaded URLs
...
def handle_duplicate_filenames(save_dir, file_name):
...
def main():
    ...
    global GITHUB_REPO, BRANCH_NAME  # Declare global variables
    ...
    # Set the branch name based on user input or default to 'main'
    ...
    # Display the contents of the metadata in a tabular format if there are any entries
    if metadata:
        print("\nFile Metadata:")
