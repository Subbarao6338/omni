...
def process_directory(url, local_dir, files_metadata):
    contents = fetch_files(url)

    # Counters for limiting downloads
    python_file_count = 0
    text_file_count = 0
    ...
    for item in contents:
        if item['type'] == 'file':
            if item['name'].endswith('.py'):
                if python_file_count >= 10:  # Limit to 10 Python files
                    continue
                local_subdir = LOCAL_PYTHON_DIR
                python_file_count += 1  # Increment counter for Python files

            elif item['name'].endswith('.txt') or item['name'].endswith('.md'):
                if text_file_count >= 10:  # Limit to 10 text files
                    continue
                local_subdir = LOCAL_TEXT_DIR
                text_file_count += 1  # Increment counter for text files
            ...
            process_directory(subdirectory_url, local_dir, files_metadata)
...
def main():
    ...
    process_directory(BASE_URL, LOCAL_PYTHON_DIR, files_metadata)
