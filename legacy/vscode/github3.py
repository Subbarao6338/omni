...
async def download_file(session, file_info, local_dir):
    async with semaphore:  # Limit concurrent downloads
        ...
        try:
            ...
            return file_path, file_info  # Return the path and file info on success
...
async def process_directory(session, url, local_dir, files_metadata, file_counts):
    ...
    for item in contents:
        if item['type'] == 'file':
            ...
            # Create tasks for downloading the file
            tasks.append(download_file(session, item, local_subdir))

        elif item['type'] == 'dir':
            # Recursively process the subdirectory
            subdirectory_url = item['url']
            tasks.append(process_directory(session, subdirectory_url, local_dir, files_metadata, file_counts))

    # Wait for all tasks to complete
    downloaded_files = await asyncio.gather(*tasks)

    # Process metadata for downloaded files
    for result in downloaded_files:
        if result is not None:  # Check if result is not None
            local_path, item_info = result  # Unpack the result
            if local_path and item_info:  # Check if both values are valid
                metadata = await get_file_metadata(session, item_info)
                if metadata:
                    files_metadata.append(metadata)
        else:
            print("A download task returned None, skipping metadata retrieval.")
